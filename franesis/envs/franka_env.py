...
class FrankaEnv(FrankaCore):
    def __init__(
        self, episode_length_s: float = 3.0, freq: int = 100, render: bool = True, device: str = "cuda"
    ) -> None:
        # 3. init robot core
        super().__init__(num_envs=1, episode_length_s=episode_length_s, freq=freq, render=render, device=device)
        self.max_episode_length = math.ceil(episode_length_s / self.ctrl_dt)

    def reset_idx(self, mask: torch.Tensor) -> None:
        if len(mask) == 0:
            return
        self.steps[mask] = 0
    ...
    def obs(self) -> tuple[torch.Tensor, dict]:
        obs_dict = {"q": self._get_q(), "dq": self._get_dq()}
        return obs_dict
